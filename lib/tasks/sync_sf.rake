...
namespace :sync_sf do
  ...
  task :generate_hcm_psndoc_csv, [:csv_file_path] => [:environment] do |_task, args|
    csv_file_path = args[:csv_file_path] || "thapeemployee_#{Date.today.strftime("%m%d%Y")}.csv"
    ...
    CSV.open(csv_file_path, 'w') do |csv|
      csv << %w[STATUS USERID USERNAME FIRSTNAME LASTNAME MI GENDER EMAIL MANAGER
        ...
        JOBFAMILY JOBLEVEL DEFAULT_LOCALE JOBROLE LOGIN_METHOD MATRIX_MANAGER STCODE]
      NcHcmAccount.all.each do |n|
        values = []
        values << n.status
        values << n.userid
        values << n.username
  ...
  task :generate_hcm_background_csv, [:csv_file_path] => [:environment] do |_task, args|
    csv_file_path = args[:csv_file_path] || "Background_#{Date.today.strftime("%m%d%Y")}.csv"
